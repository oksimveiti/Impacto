...
export async function getEventPetitionDetails(eventId) {
  if (!eventId) {
  ...
  try {
    ...
    // イベントの日時表示
    const startDate = eventData.event_start_date_time.toDate();
    const endDate = eventData.event_end_date_time.toDate();
    document.getElementById("startDateTime").textContent =
      startDate.toLocaleString("en-US", dateOptions);
    document.getElementById("endDateTime").textContent = endDate.toLocaleString(
      "en-US",
      dateOptions
    );

    // 場所の表示
    document.getElementById("eventNeighborhood").textContent = `
      ${eventData.neighborhood},
      ${eventData.city},
      ${eventData.province}`;
    document.getElementById("fullAddress").textContent = eventData.address;
    ...
    eventData.event_categories.forEach((category) => {
      ...
      tag.textContent = CATEGORY_DISPLAY_NAMES[category] || category;
    ...
    document.getElementById("petitionStartDate").textContent =
      petitionStartDate.toLocaleString("en-US", dateOptions);
    document.getElementById("petitionEndDate").textContent =
      petitionEndDate.toLocaleString("en-US", dateOptions);

    // サポーター数の表示とプログレスバーの更新
    const supporters = eventData.supporters || [];
    const maxSupporters = eventData.max_supporters;
    const supporterCount = supporters.length;
    const supporterPercentage = (supporterCount / maxSupporters) * 100;

    document.getElementById(
      "petitionTarget"
    ).textContent = `${supporterCount.toLocaleString()} / ${maxSupporters.toLocaleString()} signatures`;
    ...
    // 参加者情報の表示
    document.getElementById("maxattendings").textContent =
      eventData.max_participants;

    // 現在の参加者数を表示
    const participants = eventData.participants || [];
    document.getElementById("currentattendings").textContent =
      participants.length;
    console.log("Current participants:", participants.length); // デバッグ用

    // 参加者のアバターを表示
    const attendeesAvatorsDiv = document.getElementById("attendeesAvators");
    const MAX_VISIBLE_AVATARS = 20; // 表示する最大アバター数
    ...
    try {
      ...
      const participantPromises = participants
        ...
        .map(async (userId) => {
          const profileSetupRef = firestoreDoc(db, "profile_setup", userId);
          const profileSetupDoc = await getDoc(profileSetupRef);
          return {
            userId,
            profilePicture: profileSetupDoc.exists()
              ? profileSetupDoc.data().basicInfo.profilePictureUrl
              : "../../../media/impacto-logo.png",
          };
        });

      const participantProfiles = await Promise.all(participantPromises);

      // アバターを表示
      attendeesAvatorsDiv.innerHTML = participantProfiles
        .map(
          (profile) => `
          <img 
      ...
      // 残りの参加者数を表示（5人以上の場合）
      if (participants.length > MAX_VISIBLE_AVATARS) {
        attendeesAvatorsDiv.innerHTML += `
    ...
    // 地図の初期化
    initMap(eventData.location, eventData.address);
