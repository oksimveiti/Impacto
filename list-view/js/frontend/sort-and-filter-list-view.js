...
// import { generateAllEventArray } from "../../../map-exploration/js/frontend/sort-and-filter.js";
...
function buttonfilterEvents(event) {
  ...
  if ( selectedCategoryBtns.length === 0 ) {
    ...
    for (let eachEvent of uniqueAllEventsArray) {
  ...
  const uniqueExtractedEvents = [...new Set(extractedEvents)]

  for (let eachEvent of uniqueExtractedEvents) {
    createEventCard(eachEvent, eachEvent.id, "event-cards-all");
  }
}
...
function filterEvents(event) {
  ...
  setTimeout(() => {
      ...
      if (checkBox.checked) {
        switch (label.textContent) {
          ...
          case "Petition":
            ...
            break;
          case "Event & Petition":
            selectedTypes.push("event_petition");
            break;
        }
      }
    }
    ...
    //Create filtered cards
    const uniqueFilteredEvents = [...new Set(filteredEvents)]
    for (let eachEvent of uniqueFilteredEvents) {
      createEventCard(eachEvent, eachEvent.id, "event-cards-all");
    }
  }, 0);
}
